...
class Admin extends ActiveRecord
{
  ...
  public function rules()
  {
    return [
      ...
      ['admin_pass', 'required', 'message' => '管理员密码不能为空', 'on' => ['login', 'changePass', 'adminAdd', 'changeEmail', 'updatePass']],
      ...
      ['admin_pass', 'validatePass', 'on' => ['login', 'changeEmail','updatePass']],
      ['rePass', 'compare', 'compareAttribute' => 'admin_pass', 'message' => '两次密码输入不一致', 'on' => ['changePass', 'adminAdd']],
      ['rePass', 'required', 'message' => '确认密码不能为空', 'on' => ['changePass', 'adminAdd', 'updatePass']],
      ['admin_email', 'unique', 'message' => '电子邮箱已被注册', 'on' => ['adminAdd', 'changeEmail']],
      ['admin_user', 'unique', 'message' => '管理员账号已被注册', 'on' => ['adminAdd']],
      ['rePass', 'compare', 'compareAttribute' => 'newPass', 'message' => '两次密码输入不一致', 'on' => ['updatePass']],
      ['newPass', 'required', 'message' => '新密码不能为空', 'on' => ['updatePass']],
    ];
  }
  ...
  public function updatePass($data)
  {
    ...
    if ($this->load($data) && $this->validate()) {
      Yii::$app->session->setFlash('info','修改密码为当前可用密码');
     return (bool)$this->updateAll(['admin_pass' => md5($this->newPass)], 'admin_user=:user', [':user' => $this->admin_user]);
    }
    return false;
  }
}
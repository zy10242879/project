                        ...
                        <div class="row">
                            <div class="col-12 col-xs-12 col-sm-12">
                              <ul>
                                ...
                                <li><a href="<?=yii\helpers\Url::to(['product/index','cate_id'=>$child['cate_id']]);?>"><?=$child['title'];?></a></li>
                                <?php endforeach;?>
                              </ul>
                            </div><!-- /.col -->
                  ...
                  <li>
                    <div class="row">
                      ...
                      <div class="col-xs-12 col-sm-3 no-margin">
                        <a href="<?=yii\helpers\Url::to(['product/detail','product_id'=>$pro->product_id]);?>" class="thumb-holder">
                          <img alt="" src="" data-echo="//<?php echo $pro->cover ?>-picsmall" />
                        </a>
                      </div>
                    </div>
                  </li>
...
<script>
  ...
  function reduce(cart_id) {
    var num = parseInt($("#cart_id"+cart_id+"aa").val())-1;
    if(num<1){
      num = 1;
      $("#cart_id"+cart_id+"aa").class('display');
    }else{
      $("#cart_id"+cart_id+"aa").val(num);
    }
...
<script>
  ...
  $('#addto-cart').click(function () {
    ...
    var price = parseFloat($("#confirm_price").html());
    $.post('<?=yii\helpers\Url::to(['cart/add']);?>',{'product_id':product_id,'_csrf-frontend':csrf,'product_num':product_num,'price':price},function (data) {
      toastr.success("成功加入购物车!"); //此处在php页中需要加入判断添加情况
      var total = parseFloat($("#cart_total").html());
      var p = total + price*product_num;
      $("#cart_total").html(p+ "");
      if(data==1){
  ...
  function del_cart(cart_id) {
    ...
    $.get(url,data,function ($e) {
      if($e==1){
        ...
        if(isNaN(num)){
          num = parseInt($('#cart_id'+cart_id+'aa').val());
        }
        var total = parseFloat($("#cart_total").html());
  ...
  function addCart(pid) {
    $.get('<?php echo yii\helpers\Url::to(['cart/add']) ?>', {'product_id':pid}, function(data) {
      ...
      if(isNaN(price)){
        price = parseFloat($("#product_price"+pid).html());
      }
      var p = total + price;
      $("#cart_total").html(p+ "");
      if(data==1){

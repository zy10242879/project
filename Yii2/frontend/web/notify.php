<?php
  //通知的回调文件的写法  curl使用
  //定义跳转地址
  $url = "http://niuniuma.com/index.php?r=pay/notify";
  //接收参数
  $post_data = $_POST;
  //使用curl来请求
  //1.初始化curl
  $ch = curl_init();
  //2.设置url和相应的选项
  curl_setopt($ch,CURLOPT_URL,$url);//设置需要获取数据的url地址
  curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);//以文件流的形式返回
  curl_setopt($ch,CURLOPT_POST,1);//启用时会发送一个常规的POST请求,就像表单提交的一样。
  curl_setopt($ch,CURLOPT_POSTFIELDS,$post_data);//全部数据使用HTTP协议中的"POST"操作来发送。
  //3.抓取URL并把它传递给浏览器
  $out = curl_exec($ch);
  //4.关闭cURL资源，并且释放系统资源
  curl_close($ch);
